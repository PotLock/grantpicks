...
use crate::{admin::{self, read_contract_owner}, data_type::{Project, ProjectParams, UpdateProjectParams}};
...
pub fn validate_update_project(update_params: &UpdateProjectParams){
  ...
  assert!(
      !update_params.overview.is_empty(),
      "overview is required"
  );
  assert!(
      !update_params.contacts.is_empty(),
      "contacts is required"
  );
  assert!(
      !update_params.image_url.is_empty(),
      "image_url is required"
  );
}
...
pub fn validate_owner_or_admin(admin: &Address, project: &Project){
  if &project.owner != admin {
    ...
    assert!(is_admin.is_some(), "only owner or admin can performe action");
  }
}
