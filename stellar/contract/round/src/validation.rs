...
use crate::{approval_writer::{is_project_approved, read_approved_projects}, data_type::{CreateRoundParams, RoundDetail}, project_registry_writer::read_project_contract, utils::get_ledger_second_as_millis, voter_writer::{is_black_listed, is_white_listed}, voting_writer::get_voting_state};
...
pub fn validate_can_payout(env: &Env, round: &RoundDetail) {
  ...
  assert!(current_time >= round.voting_end_ms, "Voting period is not ended");
}
...
pub fn validate_vault_fund(round: &RoundDetail) {
  ...
  assert!(
      vault_fund > 0,
      "Vault fund must be greater than 0"
  );
}
