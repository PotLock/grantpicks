...
function loadPerNetworkConfig(): Record<Network, PerNetworkConfig> {
  ...
  const match = content.match(/const perNetworkConfig:[\s\S]*?=\s*\{([\s\S]*?)\}\n/)
  ...
  const normalized = objLiteral
    ...
    .replace(/(LISTS_CONTRACT_ID|PROJECT_REGISTRY_CONTRACT_ID|ROUND_CONTRACT_ID|API_BASE_URL)\s*:/g, '"$1":')

  // eslint-disable-next-line no-new-func
  const parsed = Function('return ' + normalized)() as Record<Network, PerNetworkConfig>
  return parsed
}
...
function ensureNetworksMap(filePath: string, contractId: string) {
  ...
  const hasPublic = /public:\s*\{[\s\S]*?networkPassphrase:[\s\S]*?Public Global Stellar Network/.test(content)
...
function main() {
  ...
  const projectPath = path.join(root, 'project-registry-client', 'src', 'index.ts')
