	...
	const onSubmit: SubmitHandler<FormValues> = async (data) => {
		...
		try {
			...
			if (listId) {
				const txUpdateList = await updateList(
					{
						...
						default_registration_status: data.approve_applications
							? ({ tag: 'Approved', values: undefined } as RegistrationStatus)
							: ({ tag: 'Pending', values: undefined } as RegistrationStatus),
				...
				const txCreateList = await createList(
					stellarPubKey,
					{
						name: data.name,
						description: data.description,
