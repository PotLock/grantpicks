...
const RoundCardContent: React.FC<RoundCardContentProps> = ({
  ...
  const closesText = () => {
    if (selectedRoundType === 'upcoming' && (currentTime === 'upcoming' || currentTime === 'upcoming-open')) {
  ...
  return (
    <>
      ...
      <div className="mb-2 flex items-center justify-between gap-2 min-w-0">
        <p className="text-base font-semibold text-grantpicks-black-950 flex-shrink-0">Funding Progress</p>
        <p className="text-sm sm:text-base font-semibold text-grantpicks-black-950 text-right truncate max-w-[60%] sm:max-w-[65%]" title={fundedDisplay()}>
          {fundedDisplay()}
        </p>
      </div>
      ...
      <div className="flex items-center mb-4 justify-between flex-wrap gap-x-2 gap-y-1">
        ...
        <div className="flex items-center space-x-2 min-w-0">
          ...
          <p className="text-sm font-normal text-grantpicks-black-950 whitespace-nowrap">{closesText()}</p>
        </div>
      </div>
    </>
  )
