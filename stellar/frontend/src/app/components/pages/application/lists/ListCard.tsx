...
export type APIListExternal = {
  ...
  updated_at: string
  created_at: string
  default_registration_status: string
...
type ListCardProps = {
  list: APIListExternal
  chainId: string | null
  stellarPubKey: string
}


export const ListCard = ({ list, stellarPubKey }: ListCardProps) => {
  const router = useRouter()
  return (
    <div
      onClick={() => router.push(`/list/${list.on_chain_id}`)}
        ...
        <div className="absolute top-2 right-2">
          <div className="bg-white/90 backdrop-blur-sm rounded-full px-2 py-1">
            <span className="text-xs font-semibold text-gray-700">
              {Number(list.registrations_count)} Projects
            </span>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex flex-row items-center gap-x-1">
              ...
              <p className="text-xs font-semibold text-gray-900">
                {list.owner?.id ? prettyTruncate(list.owner?.id || '', 8, 'address') : ""}
              </p>
            </div>
          </div>
