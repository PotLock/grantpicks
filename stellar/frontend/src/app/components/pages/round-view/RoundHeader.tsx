			...
			<div className="flex items-center justify-between gap-4 flex-wrap">
				<div className="flex items-center gap-2">
					<IconArrowLeft
						...
						onClick={onGoBack}
					/>
					<h1 className="text-2xl md:text-[32px] font-semibold text-grantpicks-black-950">
						{name}
					</h1>
				</div>
				<div className="flex items-center gap-2">
					...
					<Button
						isDisabled={!doc?.use_vault}
						color="white"
						onClick={openFundModal}
					>
						{doc?.use_vault ? 'Fund Round' : 'Funding Not Allowed'}
					</Button>
...
export function resolveRoundTimelineStatus(
	doc?: GPRound,
	nowDate?: Date,
): RoundTimelineStatus {
	const now = (nowDate ?? new Date()).getTime()
	const appStart = doc?.application_start
		? new Date(doc.application_start).getTime()
		: NaN
	const appEnd = doc?.application_end
		? new Date(doc.application_end).getTime()
		: NaN
	const votingStart = doc?.voting_start
		? new Date(doc.voting_start).getTime()
		: NaN
	...
	if (hasApplication) {
		...
		if (now >= appStart && now < appEnd)
			return RoundTimelineStatus.APPLICATION_OPEN
		if (!Number.isNaN(votingStart) && now < votingStart)
			return RoundTimelineStatus.APPLICATION_ENDED
		if (
			!Number.isNaN(votingStart) &&
			!Number.isNaN(votingEnd) &&
			now >= votingStart &&
			now < votingEnd
		)
