...
const RoundAdminsPage = () => {
  ...
  if (isLoading) {
    return <div className="text-center text-grantpicks-black-950">Loading Round Admins</div>
  }
    ...
    <div>
      <div className="flex mb-2 items-center justify-between w-full">
        <div className="flex items-center w-full justify-between mb-4 gap-2">
          <h2 className="text-xl font-bold text-grantpicks-black-950 uppercase">Round Admins</h2>
          {stellarPubKey === data?.owner?.id && (
            <div
              ...
              <button
                ...
                className="rounded-full w-8 h-8 flex items-center justify-center bg-grantpicks-alpha-50/5 cursor-pointer hover:opacity-70 transition"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {data?.admins?.length && data?.admins?.length > 0 ? data?.admins.map((admin, idx) => (
          <AdminCard key={idx} address={admin} />
        )) : (
          <div className="col-span-full">
            <div className="text-center border border-black/10 rounded-xl p-8 bg-white">
              <p className="text-base font-semibold text-grantpicks-black-950 mb-1">No admins found</p>
              <p className="text-sm text-grantpicks-black-600">This round has no admins.</p>
            </div>
          </div>
        )}
