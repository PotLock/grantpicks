...
const RoundApplicationsPage = () => {
  ...
  const { data, isLoading, error, mutate } = useSWR(`/rounds/${roundId}/applications`,
  ...
  if (error) {
    return <div className="text-center text-grantpicks-black-950">Error Loading Applications</div>
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6">
      <div className="sticky top-0 z-10 -mx-4 md:-mx-6 px-4 md:px-6 bg-white pt-4 pb-3 border-b border-black/10">
        <div className="flex items-center overflow-x-auto space-x-2 md:space-x-3" role="tablist" aria-label="Application filters">
          <button
            onClick={() => setTab('all')}
            className={clsx(
        ...
        <div className="flex items-center space-x-2 mt-3">
          <p className="text-xs font-semibold text-grantpicks-black-600 uppercase">
            <span className="text-sm font-bold text-grantpicks-black-950 mr-1">{roundAppsData.length}</span>
            {tab === 'all' ? 'applications' : tab}
          </p>
        </div>
      </div>
      <div className="py-4 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
        {roundAppsData?.length > 0 ? roundAppsData?.map((application, idx) => (
          <ApplicationItem key={idx} roundData={application?.round} type={application.status} index={idx} item={application} mutate={mutate} />
        )) : (
          <div className="col-span-full">
            <div className="text-center border border-black/10 rounded-xl p-8 bg-white">
              <p className="text-base font-semibold text-grantpicks-black-950 mb-1">No applications found</p>
              <p className="text-sm text-grantpicks-black-600">Try switching tabs or check back later.</p>
            </div>
          </div>
        )}
      </div>
