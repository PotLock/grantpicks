...
const RoundDurationPage = () => {
	...
	const isAdmin =
		Array.isArray(data?.admins) && data?.admins.map(admin => admin.id).includes(stellarPubKey)
	...
	return (
		<div>
			<TimePeriodDrawer doc={data as unknown as Omit<GPRound, 'admins'> & { admins: { id: string }[] }} mutateRounds={mutate} />
		</div>
	)
}
