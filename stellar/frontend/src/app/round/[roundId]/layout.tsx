...
const SubNav = ({ basePath, isOwnerOrAdmin }: { basePath: string, isOwnerOrAdmin: boolean }) => {
  ...
  const tabs = [
    ...
    { href: `${basePath}/duration`, label: 'Duration Settings', show: isOwnerOrAdmin },
  ]
  return (
    <div className="flex items-center gap-6 border-b border-black/10 mt-6">
      {tabs?.filter((t) => t.show).map((t) => {
        ...
        return (
          <Link key={t.href} href={t.href} className={`relative py-3 text-sm font-semibold ${isActive ? 'text-grantpicks-black-950' : 'text-grantpicks-black-500'}`}>
            {t.label}
            {isActive && <span className="absolute left-0 -bottom-[1px] w-full h-[2px] bg-grantpicks-black-950 rounded-full" />}
          </Link>
        )
      })}
    </div>
...
const RoundLayout = ({ children, params }: { children: React.ReactNode; params: { roundId: string } }) => {
  ...
  const isOwnerOrAdmin = useMemo(() => {
    return roundInfo?.owner?.id === stellarPubKey || roundInfo?.admins.includes(stellarPubKey)
  }, [roundInfo, stellarPubKey])

  useEffect(() => {
    onFetchRoundInfo().then((roundInfo) => {
  ...
  const closesText = useMemo(() => {
    const now = new Date()
    const appStart = roundInfo?.application_start ? new Date(roundInfo.application_start) : undefined
    const appEnd = roundInfo?.application_end ? new Date(roundInfo.application_end) : undefined
    const voteStart = roundInfo?.voting_start ? new Date(roundInfo.voting_start) : undefined
    const voteEnd = roundInfo?.voting_end ? new Date(roundInfo.voting_end) : undefined
    ...
    <ApplicationLayout>
      ...
      <div className={`${isLoading ? 'opacity-50' : 'opacity-100'}`}>
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          <StatCard label="Approved Projects" value={roundInfo?.approved_projects?.length || 0} Icon={<IconProject size={24} className="fill-grantpicks-black-400" />} />
          <StatCard label="Expected Amount" value={roundInfo?.expected_amount || 0} Icon={<IconStellar size={24} className="fill-grantpicks-black-400" />} global={global} />
          <StatCard label="Total Vault Balance" value={roundInfo?.current_vault_balance || 0} Icon={<IconStellar size={24} className="fill-grantpicks-black-400" />} global={global} />
        </div>
        <div className="mt-6">
          <ProgressBar large={true} value={Number(formatStroopToXlm(BigInt(roundInfo?.vault_total_deposits || 0))) || 0} max={Number(formatStroopToXlm(BigInt(roundInfo?.expected_amount || 0))) || 0} label="Funding Progress" />
        </div>
